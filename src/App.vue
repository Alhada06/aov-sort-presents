  ...
  <div class="w-full h-full p-4 flex justify-center items-center">
    <div class="max-w-md">
      ...
      <div class="mt-2 flex justify-center items-center">
        <img v-for="present in sortedPresents" :key="present.id" :src="present.src" :alt="`Present ${present.id}`"
          data-qa="present" />
      </div>
      <button @click="next()"
        class=" bg-red-600 px-6 py-2 rounded-lg flex items-center justify-center mx-auto mt-8 hover:bg-red-700 text-white">Toggle
        sort</button>
    </div>
  </div>
</template>
